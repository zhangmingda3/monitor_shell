...
#       [ -z $directory  ];then  ##用户输入，同步到用户指定目录
	rsync -az ${rsyncip}${mulu}  $mulu2; 2>> /root/桌面/rsync.log
if	[ $? -eq 0 ]; then
	echo "$(date)${rsyncip}${mulu}同步到${mulu2}成功" >> /root/桌面/rsync.log
else
	echo "$(date)${rsyncip}${mulu}同步到${mulu2}失败" >> /root/桌面/rsync.log
